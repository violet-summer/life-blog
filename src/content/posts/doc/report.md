...
如图所示，蓝色区域为**海洋和河流**，海洋右侧紧邻棕色的**海岸线**；紫色和蓝色作为**主干道和主要道路**并且会跨过河流区域作为**桥梁**；红色为**次要道路**；绿色区域为公园，次要道路经过公园内部会进行扭曲变换形成公园**步道**；深色块代表被选中为**建筑块**进行概率性递归切分，白色块为**空地**。

![](assets/report/file-20250626002931831.png)
...
3.  **噪声扰动 (可选)**: 为了增加自然感和不规则性，向组合场中引入可控的旋转噪声（例如基于Simplex噪声），这会使场的方向在局部发生平滑的随机偏转。
...
2.  **世界边界 (`world_dimensions`, `origin`)**: 定义流线生成的有效区域，其实是世界坐标系的边界约束。
3.  **种子点策略**:
    1.   初始种子点列表 (`candidateSeedsMajor`, `candidateSeedsMinor`)，其中保存流线的双向生长起源点，并且两个方向垂直正交。最初的种子点使用随机数进行生成，后续需在生成流线的过程中会将首尾作为新的种子点进行补充。
    2.   种子点尝试次数 (`seed_tries`): 尝试寻找有效起始点的最大次数，这个次数尝试会影响道路网络密度。
4.  **积分参数**:
...
7.  **水体尺寸参数**:
    ...
    2.   `river_size`: 河流区域整体宽度 $w_{river}$。
    3. `river_size`-`river_bank_size`: 是河道区域的宽度
8.  **几何处理参数**:
    1.   `simplify_tolerance`: 简化水体边界多边形时的容差。容差越大，省略的点就越多，容差越小，保留的点越多，更加贴合曲线。
...
### 建筑底面轮廓构建（Building lot Generation）
...
### 公园及其步道生成（Park & Trail Generation）
...
![](assets/report/file-20250626003430307.png)
## 水域生成
...
![](assets/report/file-20250626003738797.png)
## 道路网络以及公园生成
...
![](assets/report/file-20250626004113943.png)
## 4、地块划分与建筑生成
...
![](assets/report/file-20250626004455395.png)
...
## streamline_interface.py & streamlines.py
...
2.   **核心类**：`ALLParams` (主配置类，采用单例模式，从JSON加载)，以及各模块的具体参数类如 `MapWaterParams`, `MainMajorMinorParams`, `MapBuildingsParams`, `TensorFieldParams` 等，均继承自Pydantic的`BaseModel`，实现类型检查、默认值和序列化。
